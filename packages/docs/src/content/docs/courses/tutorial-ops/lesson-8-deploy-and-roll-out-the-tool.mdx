...
This lesson wraps up the first verson of the HarborFlow application.

Taylor now has a tool that works locally, reflects the real exception process, and includes enough access control and automation to matter. The next step is to get it into the hands of the team.
...
The point of deployment make the application available internally.
...
Use [`lesson-8-blueprint.toml`](/tutorial-ops/checkpoints/lesson-8-blueprint.toml) as the complete final course blueprint. It is intentionally close to the Lesson 7 checkpoint because this lesson is mostly about rollout, verification, and deployment discipline.

Validate it before deploying:

```bash
zebric validate --blueprint lesson-8-blueprint.toml
```
...
In the HarborFlow narrative, Taylor has now gone from:
