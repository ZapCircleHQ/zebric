...
- On Node, verify the configured Better Auth secret resolves correctly.
- On Workers, verify `SESSION_KV` is bound and the request includes the configured session cookie. `SESSION_SECRET` is not currently consumed by the Workers runtime.
