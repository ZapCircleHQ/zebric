...
import type { Query, Form, Blueprint, Page, Theme } from '@zebric/runtime-core'
...
import type { SessionManager, UserSession } from '@zebric/runtime-core'
import type { PluginRegistry } from '../plugins/index.js'
import { ErrorSanitizer } from '@zebric/runtime-core'
import { AuditLogger, AuditEventType, AuditSeverity } from '../security/index.js'
...
export class RouteHandler {
  ...
  private async getSession(request: FastifyRequest): Promise<UserSession | null> {
    ...
    try {
      return await this.sessionManager.getSession(request as any)
    } catch (error) {
  ...
  private async checkFormAuthorization(
    ...
    const { AccessControl } = await import('@zebric/runtime-core')
